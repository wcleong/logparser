...
def main():
    ...
    parser.add_argument(
        'get_option', help='[getcodes, geturls, getUAs, getreport]')
    parser.add_argument("--code", metavar='<http_status_code>',
                        help="http error code to filter by")
    parser.add_argument("--from", dest='from_', metavar='<from_date>',
                        type=valid_date, help="start date for search")
    parser.add_argument("--to", metavar='<from_date>',
                        type=valid_date, help="end date for search")
    parser.add_argument("--max", metavar='<count>',
                        help="Top N items to display")
    parser.add_argument("--for", dest='for_', nargs=2,
                        metavar='<relative_time>', help="Relative duration")
    ...
    if args.get_option == 'getreport':
        ...
        if args.max:
            ...
            sorted_results = sorted(
                results.items(), key=lambda x: x[1], reverse=True)[:max]
        else:
            # this is an expensive operation, cap at 10 items if no max value specified
            sorted_results = sorted(
                results.items(), key=lambda x: x[1], reverse=True)[:10]

        top_status_codes = logparser.get_top_status_code_by_url(
            sorted_results, download_dir, files)
        report = logparser.get_max_processing_times_by_url(
            top_status_codes, download_dir, files)
